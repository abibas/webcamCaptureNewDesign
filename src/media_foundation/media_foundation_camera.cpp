...
namespace webcam_capture {
  ...
  MediaFoundation_Camera::MediaFoundation_Camera(frame_callback fc, void* user)
    :Base(fc, user)
    ,state(CA_STATE_NONE)
    ,mf_callback(NULL)
    ...
    ,imf_source_reader(NULL)
  {
    ...
    if(FAILED(hr)) {
        ///TODO don't know, but this error becomes every time.
  ...
  int MediaFoundation_Camera::open(Settings settings) {
...
//    }
  ...
  void MediaFoundation_Camera::update() {

  }

  std::vector<Device> MediaFoundation_Camera::getDevices() {
  ...
  std::vector<Capability> MediaFoundation_Camera::getCapabilities(int device) {
  ...
  std::vector<Format> MediaFoundation_Camera::getOutputFormats() {
